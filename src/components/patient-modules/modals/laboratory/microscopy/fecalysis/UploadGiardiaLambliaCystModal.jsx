...
const UploadGiardiaLambliaCystModal = (props, ref) => {
	...
	const submit = (data) => {
		...
		formdata.append("giardia_lamblia_cyst", data?.giardia_lamblia_cyst);
							...
							<Dialog.Panel className="w-full lg:max-w-xl transform overflow-hidden rounded-2xl bg-white text-left align-middle shadow-xl transition-all">
								...
								>
                                    ...
                                    <span className="text-sm text-center font-bold  text-blue-600 mt-2">
										Giardia Lamblia Cyst Test Result
									</span>
								</Dialog.Title>
								<div className="p-6 flex flex-col gap-y-4 relative">
									<TextInputField
										label="Giardia Lamblia Cyst"
										placeholder="70-110mg/dL"
										{...register("giardia_lamblia_cyst", {
											required:
												"The Alkaline Phos. is required.",
										})}
										errors={errors?.giardia_lamblia_cyst?.message}
									/>
									{/* <TextAreaField
										label="Remarks"
										placeholder=""
...
export default forwardRef(UploadGiardiaLambliaCystModal)
