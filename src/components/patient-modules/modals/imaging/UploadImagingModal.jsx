...
const UploadImagingModal = (props, ref) => {
							...
							<Dialog.Panel className="w-full lg:max-w-xl transform overflow-hidden rounded-2xl bg-white text-left align-middle shadow-xl transition-all">
								...
								<div className="p-6 flex flex-col gap-y-4 relative">
									<div className="border-b">
										{filteredPending.map((queue, index) => (
                                            <span key={index} className="text-xl text-center font-bold  text-blue-500">
                                                Order: {queue?.type?.name}
                                            </span>
                                        ))}
									</div>
									<Controller
...
export default UploadImagingModal
