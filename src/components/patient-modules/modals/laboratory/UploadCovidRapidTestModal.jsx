...
const UploadCovidRapidTestModal = (props, ref) => {
	...
	const submit = (data) => {
		...
		formdata.append("type_specimen", data?.type_specimen);
		formdata.append("test_kit", data?.test_kit);
							...
							<Dialog.Panel className="w-full lg:max-w-xl transform overflow-hidden rounded-2xl bg-white text-left align-middle shadow-xl transition-all">
								...
								<div className="p-6 flex flex-col gap-y-4 relative">
									<TextInputField
										...
										errors={errors?.type_specimen?.message}
									/>
									<TextInputField
										label="Test Kit Brand"
										placeholder=""
										{...register("test_kit", {
											required:
												"The Specimen Source is required.",
										})}
										errors={errors?.test_kit?.message}
									/>
									<TextInputField
										label="Method"
										placeholder=""
...
export default forwardRef(UploadCovidRapidTestModal)
