...
import MiscModal from "../../components/modal/laboratory/MiscModal";
		...
		<div className="flex flex-col gap-y-2 w-full">
			<div className="flex items-center justify-between w-full">
				<div className="italic border-b w-full flex items-center font-semibold pb-2">
					<span className="mr-4">MISCELLANEOUS</span>
					<ActionBtn
						className="text-base gap-2 !w-[250px] mr-2"
						onClick={() => {
							miscRef?.current?.show(patient);
						}}
					>
						<FlatIcon icon="rr-plus" /> Miscellaneous
					</ActionBtn>
				</div>
			</div>
			<MiscModal ref={miscRef} />
			<div className="flex items-start justify-start flex-wrap gap-3 mb-3 w-full px-0">
				<Card color="red" title="Miscellaneous" icon="result"></Card>
			</div>
		</div>
	);
};
