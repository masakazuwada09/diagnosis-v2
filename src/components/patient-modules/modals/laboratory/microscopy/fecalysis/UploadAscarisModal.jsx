...
const UploadAscarisModal = (props, ref) => {
	...
	const submit = (data) => {
		...
		formdata.append("ascaris_lumbricoides", data?.ascaris_lumbricoides);
							...
							<Dialog.Panel className="w-full lg:max-w-xl transform overflow-hidden rounded-2xl bg-white text-left align-middle shadow-xl transition-all">
								...
								>
                                    ...
                                    <span className="text-sm text-center font-bold  text-blue-600 mt-2">
										Ascaris Lumbricoides Ova Test Result
									</span>
								</Dialog.Title>
								<div className="p-6 flex flex-col gap-y-4 relative">
									<TextInputField
										label="Ascaris Lumbricoides Ova"
										placeholder="70-110mg/dL"
										{...register("ascaris_lumbricoides", {
											required:
												"The Alkaline Phos. is required.",
										})}
										errors={errors?.ascaris_lumbricoides?.message}
									/>
									{/* <TextAreaField
										label="Remarks"
										placeholder=""
...
export default forwardRef(UploadAscarisModal)
