...
const UploadMiscellaneousFormModal = (props, ref) => {
	...
	const submit = (data) => {
		...
		formdata.append("test_name_misc", data?.test_name_misc);
		formdata.append("result_misc", data?.result_misc);
		formdata.append("remarks_misc", data?.remarks_misc);
							...
							<Dialog.Panel className="w-full lg:max-w-xl transform overflow-hidden rounded-2xl bg-white text-left align-middle shadow-xl transition-all">
								...
								>
									<span className="text-xl text-center font-bold  text-blue-900">
										Miscellaneous Form Examination Result
									</span>
								...
								<div className="p-6 flex flex-col gap-y-4 relative">
									<TextInputField
										label="Test Name/Method"
										placeholder="70-110mg/dL"
										{...register("test_name_misc", {
											required:
												"The Alkaline Phos. is required.",
										})}
										errors={errors?.test_name_misc?.message}
									/>
                                    <TextInputField
										label="Result"
										placeholder="70-110mg/dL"
										{...register("result_misc", {
											required:
												"The Alkaline Phos. is required.",
										})}
										errors={errors?.result_misc?.message}
									/>
                                    <TextInputField
										label="Remarks"
										placeholder="70-110mg/dL"
										{...register("remarks_misc", {
											required:
												"The Alkaline Phos. is required.",
										})}
										errors={errors?.remarks_misc?.message}
									/>
									{/* <TextAreaField
										label="Remarks"
										placeholder=""
...
export default forwardRef(UploadMiscellaneousFormModal)
