...
const UploadTyphoidTestModal = (props, ref) => {
	...
	const submit = (data) => {
		...
		formdata.append("igg_typhoid", data?.igg_typhoid);
		formdata.append("igm_typhoid", data?.igm_typhoid);
							...
							<Dialog.Panel className="w-full lg:max-w-xl transform overflow-hidden rounded-2xl bg-white text-left align-middle shadow-xl transition-all">
								...
								<div className="p-6 flex flex-col gap-y-4 relative">
									<TextInputField
										...
										{...register("igg_typhoid", {
											required:
												"The Alkaline Phos. is required.",
										})}
										errors={errors?.igg_typhoid?.message}
									/>
                                    <TextInputField
										label="IgM"
										placeholder="70-110mg/dL"
										{...register("igm_typhoid", {
											required:
												"The Alkaline Phos. is required.",
										})}
										errors={errors?.igm_typhoid?.message}
									/>
									{/* <TextAreaField
										label="Remarks"
										placeholder=""
...
export default forwardRef(UploadTyphoidTestModal)
