...
const UploadEntamoebaHistolyticaCystModal = (props, ref) => {
	...
	const submit = (data) => {
		...
		formdata.append("entamoeba_histolytica_cyst", data?.entamoeba_histolytica_cyst);
							...
							<Dialog.Panel className="w-full lg:max-w-xl transform overflow-hidden rounded-2xl bg-white text-left align-middle shadow-xl transition-all">
								...
								>
                                    ...
                                    <span className="text-sm text-center font-bold  text-blue-600 mt-2">
										Entamoeba Histolytica Cyst Test Result
									</span>
								</Dialog.Title>
								<div className="p-6 flex flex-col gap-y-4 relative">
									<TextInputField
										...
										{...register("entamoeba_histolytica_cyst", {
											required:
												"The Alkaline Phos. is required.",
										})}
										errors={errors?.entamoeba_histolytica_cyst?.message}
									/>
									{/* <TextAreaField
										label="Remarks"
										placeholder=""
...
export default forwardRef(UploadEntamoebaHistolyticaCystModal)
