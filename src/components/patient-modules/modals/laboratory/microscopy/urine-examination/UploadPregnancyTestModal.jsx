...
const UploadPregnancyTestModal = (props, ref) => {
	...
	const submit = (data) => {
		...
		formdata.append("specimen_pregnancy_test", data?.specimen_pregnancy_test);
		formdata.append("result_pregnancy_test", data?.result_pregnancy_test);
							...
							<Dialog.Panel className="w-full lg:max-w-xl transform overflow-hidden rounded-2xl bg-white text-left align-middle shadow-xl transition-all">
								...
								<div className="p-6 flex flex-col gap-y-4 relative">
									<TextInputField
										label="Specimen"
										placeholder=""
										{...register("specimen_pregnancy_test", {
											required:
												"The hemoglobin is required.",
										})}
										errors={errors?.specimen_pregnancy_test?.message}
									/>
									<TextInputField
										label="Result"
										placeholder=""
										{...register("result_pregnancy_test", {
											required:
												"The hematocrit is required.",
										})}
										errors={errors?.result_pregnancy_test?.message}
									/>
									{/* <TextInputField
										label="Remarks"
										placeholder=""
...
export default forwardRef(UploadPregnancyTestModal)
