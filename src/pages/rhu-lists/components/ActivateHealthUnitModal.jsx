...
const ActivateHealthUnitModal = (props, ref) => {
	...
	const submit = (data) => {
		...
		let url = `v1/health-unit/activate`;
		if (healthUnit?.id) {
			url = `v1/health-unit/activate/${healthUnit?.id}`;
			formData.append("_method", "PATCH");
		}
		Axios.post(url, formData).then((res) => {
			setTimeout(() => {
				toast.success("Health Unit activated successfully!");
				if (onSuccess) {
					onSuccess();
				}
			}, 200);
...
export default forwardRef(ActivateHealthUnitModal);
