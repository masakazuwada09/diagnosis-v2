...
const UploadErythrocyteModal = (props, ref) => {
	...
	const submit = (data) => {
		...
		formdata.append("erythrocyte_arte_hematology", data?.erythrocyte_arte_hematology);
							...
							<Dialog.Panel className="w-full lg:max-w-xl transform overflow-hidden rounded-2xl bg-white text-left align-middle shadow-xl transition-all">
								...
								>
                                    ...
                                    <span className="text-sm text-center font-bold  text-blue-600 mt-2">
										Erythrocyte Sedimentation Rate - (ESR) Test Result
									</span>
								</Dialog.Title>
								<div className="p-6 flex flex-col gap-y-4 relative">
									<TextInputField
										...
										placeholder="mm/Hr"
										{...register("erythrocyte_arte_hematology", {
											required:
												"The hemoglobin is required.",
										})}
										errors={errors?.erythrocyte_arte_hematology?.message}
									/>
									{/* <TextAreaField
										label="Remarks"
										placeholder=""
...
export default forwardRef(UploadErythrocyteModal)
