...
const UploadLabResultModal = (props, ref) => {
	...
	const show = (data) => {
		...
		console.log("/v1/imaging/tests/list");
		setShowData(data);
		setModalOpen(true);
	};
	const hide = () => {
	...
	const submit = (data) => {
		...
		formData.append("imaging_result_notes", data?.lab_result_notes);
		...
		// return "";
		Axios.post(
			`v1/imaging/upload-imaging-result/${showData?.id}`,
			formData,
			config
		)
			.then((res) => {
...
export default forwardRef(UploadLabResultModal);
