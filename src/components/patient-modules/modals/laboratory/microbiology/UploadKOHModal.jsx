...
const UploadKOHModal = (props, ref) => {
	...
	const submit = (data) => {
		...
		formdata.append("specimen_koh", data?.specimen_koh);
		formdata.append("result_koh", data?.result_koh);
		formdata.append("remarks_koh", data?.remarks_koh);
							...
							<Dialog.Panel className="w-full lg:max-w-xl transform overflow-hidden rounded-2xl bg-white text-left align-middle shadow-xl transition-all">
								...
								<div className="p-6 flex flex-col gap-y-4 relative">
									<TextInputField
										label="Specimen"
										placeholder=""
										{...register("specimen_koh", {
											required:
												"The Specimen Type is required.",
										})}
										errors={errors?.specimen_koh?.message}
									/>
									<TextInputField
										label="Result"
										placeholder=""
										...
										errors={errors?.result_koh?.message}
									/>
									<TextInputField
										label="Remarks"
										placeholder=""
										{...register("remarks_koh", {
											required: "The Remarks is required.",
										})}
										errors={errors?.remarks_koh?.message}
									/>
								</div>
...
export default forwardRef(UploadKOHModal)
