...
const UploadBilirubinModal = (props, ref) => {
	...
	const submit = (data) => {
		...
		formdata.append("total_bilirubin", data?.total_bilirubin);
		formdata.append("direct_bilirubin", data?.direct_bilirubin);
		formdata.append("indirect_bilirubin", data?.indirect_bilirubin);
							...
							<Dialog.Panel className="w-full lg:max-w-xl transform overflow-hidden rounded-2xl bg-white text-left align-middle shadow-xl transition-all">
								...
								<div className="p-6 flex flex-col gap-y-4 relative">
									<TextInputField
										...
										placeholder="0.2-1.2mg/dL"
										{...register("total_bilirubin", {
											required:
												"The Total Bilirubin is required.",
										})}
										errors={errors?.total_bilirubin?.message}
									/>
                                    <TextInputField
										label="Direct Bilirubin"
										placeholder="< 0.2mg/dL"
										{...register("direct_bilirubin", {
											required:
												"The Direct Bilirubin is required.",
										})}
										errors={errors?.direct_bilirubin?.message}
									/>
                                    <TextInputField
										label="Indirect Bilirubin"
										placeholder="0.2-1.2mg/dL"
										{...register("indirect_bilirubin", {
											required:
												"The Indirect Bilirubin is required.",
										})}
										errors={errors?.indirect_bilirubin?.message}
									/>
									{/* <TextAreaField
										label="Remarks"
										placeholder=""
...
export default forwardRef(UploadBilirubinModal)
