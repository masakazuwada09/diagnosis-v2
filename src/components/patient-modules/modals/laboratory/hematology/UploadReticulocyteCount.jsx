...
const UploadReticulocyteCount = (props, ref) => {
	...
	const submit = (data) => {
		...
		formdata.append("reticulocyte_count_hematology", data?.reticulocyte_count_hematology);
							...
							<Dialog.Panel className="w-full lg:max-w-xl transform overflow-hidden rounded-2xl bg-white text-left align-middle shadow-xl transition-all">
								...
								<div className="p-6 flex flex-col gap-y-4 relative">
									<TextInputField
										...
										placeholder="%"
										{...register("reticulocyte_count_hematology", {
											required:
												"The hemoglobin is required.",
										})}
										errors={errors?.reticulocyte_count_hematology?.message}
									/>
									<TextAreaField
										label="Remarks"
										placeholder=""
...
export default forwardRef(UploadReticulocyteCount)
