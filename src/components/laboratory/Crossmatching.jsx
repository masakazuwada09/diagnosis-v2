...
import CrossmatchingModal from "../../components/modal/laboratory/CrossmatchingModal";
...
const Crossmatching = (props) => {
  const {patient} = props;
  const crossmatchRef = useRef(null);
  return (
    <div className="flex flex-col gap-y-2 w-full">
      <div className="flex items-center justify-between w-full">
    <div className="italic border-b w-full flex items-center font-semibold pb-2"> 
    <span className="mr-4">CROSSMATCHING</span>
    <ActionBtn
			className="text-base gap-2 !w-[250px] mr-2"
			onClick={() => {
				crossmatchRef?.current?.show(patient);
			}}
		>  
			<FlatIcon icon="rr-plus" /> Crossmatching
		</ActionBtn>
    </div>
    </div>
    <CrossmatchingModal 
    ...
    <div className="flex items-start justify-start flex-wrap gap-3 mb-3 w-full px-0">
      <Card
      color="red"
      title="Cross Matching Examination Result"
      icon="result"
      >
      </Card>
      </div>
