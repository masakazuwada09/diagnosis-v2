...
const ReferToInfectiousModal = (props, ref) => {
	...
	const submit = (data) => {
		...
		Axios.post(`/v1/infectious/refer-to-infectious/${appointment?.id}`, formData)
			.then((res) => {
				// addToList(data);
				setTimeout(() => {
					setLoading(false);
					// onSuccess();
					toast.success("Patient successfully referred to Infectious!");
				}, 400);
				hide();
			})
			.finally(() => {
...
export default forwardRef(ReferToInfectiousModal);
