...
const UploadChemicalExamModal = (props, ref) => {
	...
	const submit = (data) => {
		...
		formdata.append("reaction_urine", data?.reaction_urine);
							...
							<Dialog.Panel className="w-full lg:max-w-xl transform overflow-hidden rounded-2xl bg-white text-left align-middle shadow-xl transition-all">
								...
								<div className="p-6 flex flex-col gap-y-4 relative">
									<TextInputField
										label="Reaction (pH)"
										placeholder=""
										{...register("reaction_urine", {
											required:
												"The Specimen Type is required.",
										})}
										errors={errors?.reaction_urine?.message}
									/>
									<TextInputField
										label="Sp. Gravity"
										placeholder=""
...
export default forwardRef(UploadChemicalExamModal)
